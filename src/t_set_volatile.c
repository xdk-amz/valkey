...
bool setTypeNextVolatile(vsetIterator *iter, smember **member) {
    return vsetNext(iter, (void **)member);
}
...
void setTypeUpdateHashtableMemberExpiry(robj *o, hashtable *ht, smember *m, mstime_t current, mstime_t expiry) {
    ...
    if (expiry == EXPIRY_NONE) {
        setTypeUntrackMember(o, m);
        smember *updated = smemberSetExpiry(m, expiry);
        if (updated != m) serverAssert(hashtableReplaceReallocatedEntry(ht, m, updated));
        return;
    }
...
int setTypeAddWithExpiry(robj *o, sds member, mstime_t expiry, int flags, bool *replaced_expired, bool *ttl_changed) {
    ...
    if (objectGetEncoding(o) == OBJ_ENCODING_HASHTABLE) {
        ...
        /* SMOVE already paid the two keyspace lookups. Use the parent's
         * insert-position probe to combine destination membership and insertion. */
        if (flags & SET_ADD_USE_INSERT_POSITION) {
            void *existing = NULL;
            ...
            setTypeIgnoreTTL(o, false);
            if (!smemberIsExpired(existing) || getExpirationPolicyWithFlags(0) == POLICY_IGNORE_EXPIRE) return 0;
            /* An expired physical member still uses the strict pop/unindex/insert
             * replacement path below. */
        }

        setTypeIgnoreTTL(o, true);
        void **member_ref = hashtableTwoPhasePopFindRef(ht, member, &position);
        if (member_ref != NULL) {
            ...
            if (!expired) {
                /* Finish the two-phase operation without deleting by resuming rehashing directly. */
                hashtableResumeRehashing(ht);
                setTypeIgnoreTTL(o, false);
                if (!(flags & SET_ADD_KEEP_EXPIRY)) {
                    setTypeUpdateHashtableMemberExpiry(o, ht, m, current, expiry);
            ...
            smemberFree(m);
            if (replaced_expired) *replaced_expired = true;
        }
        setTypeIgnoreTTL(o, false);
    ...
    if (objectGetEncoding(o) == OBJ_ENCODING_LISTPACK) {
        ...
        if (p != NULL) {
            ...
            if (listpackObjectItemIsValid(current)) {
                if (!(flags & SET_ADD_KEEP_EXPIRY) && current != expiry) {
                    setTypeListpackSetExpiry(o, p, expiry);
